...
public class CollectTask extends TimerTask {
    ...
    public void run() {
        ...
        try {
            File temporary=new File("g:/logs/temporary");
            ...
            Path hdfs=new Path("/logs/"+datastr);
            if(!fs.exists(hdfs)){
                fs.mkdirs(hdfs);
            }
            logger.debug("begin to move files to hdfs.");
            for(File file:files){
                fs.copyFromLocalFile(new Path(file.getAbsolutePath()),new Path("/logs/"+datastr+"/"+ UUID.randomUUID()+".log"));
                FileUtils.moveFileToDirectory(file, new File("g:/logs/copys"), true);
            }
